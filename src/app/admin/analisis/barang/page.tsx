...
import { Search, Loader2, Package, Banknote, Building2, BarChart3, TrendingUp } from "lucide-react";
import {
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-lg text-sm">
        ...
        <p className="text-slate-600">
          {payload[0].name === "totalPagu" 
            ? formatRupiahSingkat(payload[0].value) 
            : `${payload[0].value} tender`}
        </p>
      </div>
    );
  }
...
export default function AnalisisBarangPage() {
  const [data, setData] = useState<BarangInsights | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/analisis?tipe=barang`);
      const json = await res.json();
      if (res.ok) {
        setData(json);
      } else {
        toast.error("Gagal mengambil data barang: " + json.error);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Analisis Barang</h1>
          <p className="text-sm text-slate-500 font-medium mt-1">
            Data khusus pengadaan barang (Teknologi, Kesehatan, Pendidikan, Otomotif).
          </p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="space-y-4">
          ...
          <div className="h-80 bg-slate-200 animate-pulse rounded-2xl"></div>
        </div>
        ...
        <div className="h-64 flex flex-col items-center justify-center text-slate-400 space-y-3 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm font-medium">Belum ada data pengadaan barang.</p>
        </div>
      ) : (
        <>
          {/* ── KPI Cards ── */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">keseluruhan data pengadaan barang</div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">akumulasi seluruh tender barang</div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">per tender pengadaan barang</div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Tren Tender Barang (6 Bulan Terakhir)</h3>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col" style={{ maxHeight: "560px" }}>
            <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Daftar Tender Barang (Top 100)</h3>
              </div>
