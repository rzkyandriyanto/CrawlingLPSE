/**
 * Email Template: Seleno Daily Digest
 * Menghasilkan HTML email premium untuk notifikasi tender harian
 */

/**
 * @param {object} user - { nama_lengkap, perusahaan }
 * @param {Array} tenders - Array tender relevan
 * @returns {string} HTML string
 */
function buildDailyDigestHtml(user, tenders) {
  const userName = user.nama_lengkap || user.perusahaan || "Pengguna";
  const today = new Date().toLocaleDateString("id-ID", {
    weekday: "long", day: "numeric", month: "long", year: "numeric"
  });

  const tenderRows = tenders.map((t, i) => {
    const scoreColor = t.score >= 70 ? "#16a34a" : t.score >= 50 ? "#d97706" : "#6366f1";
    const scoreBg = t.score >= 70 ? "#f0fdf4" : t.score >= 50 ? "#fffbeb" : "#eef2ff";
    return `
    <tr>
          ...
          <tr>
            <td style="padding: 16px 20px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    <p style="margin: 0 0 4px 0; font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 1.5px;">Tender #${i + 1}</p>
                    <p style="margin: 0 0 10px 0; font-size: 15px; font-weight: 700; color: #1e293b; line-height: 1.4;">${t.tenderName}</p>
                  </td>
                  <td style="text-align: right; vertical-align: top; white-space: nowrap; padding-left: 12px;">
                    <span style="display: inline-block; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 800; color: ${scoreColor}; background: ${scoreBg};">
                      ${t.score}% cocok
                    </span>
                  </td>
                </tr>
...
<head>
  ...
  <title>Seleno Daily Digest - ${today}</title>
</head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; width: 100%;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #1e1b4b 0%, #312e81 60%, #4c1d95 100%); border-radius: 20px 20px 0 0; padding: 32px 40px; text-align: center;">
              ...
              <p style="margin: 0; font-size: 12px; color: rgba(255,255,255,0.6); letter-spacing: 2px; text-transform: uppercase;">Daily Digest</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background: #6366f1; padding: 18px 40px;">
              <p style="margin: 0; font-size: 14px; color: #e0e7ff;">
                <strong style="color: #ffffff;">${today}</strong>&nbsp;&nbsp;·&nbsp;&nbsp;
                ${tenders.length} tender relevan baru untuk Anda
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background: #f8fafc; padding: 32px 40px;">
              <p style="margin: 0 0 24px 0; font-size: 16px; color: #334155; line-height: 1.6;">
                ...
                Berikut adalah ringkasan tender pengadaan pemerintah yang kami temukan hari ini dan sesuai dengan profil bisnis Anda.
              </p>
          ...
          <tr>
            <td style="background: #f1f5f9; border-radius: 0 0 20px 20px; padding: 24px 40px; text-align: center; border-top: 1px solid #e2e8f0;">
              ...
              <p style="margin: 0; font-size: 11px; color: #cbd5e1;">
                Anda menerima email ini karena terdaftar sebagai pengguna aktif Seleno.
              </p>
            </td>
          </tr>
...
module.exports = { buildDailyDigestHtml };
