...
const DAFTAR_BIDANG = ['Teknologi', 'Otomotif', 'Konstruksi', 'Kesehatan', 'Logistik', 'Pangan']
...
export default function PilihBidangPage() {
  ...
  const handleSimpan = async () => {
    ...
    if (error) {
      ...
      // Hapus user lama agar saat login ulang data 'tag' terbaru terambil
    ...
    <div className="relative min-h-screen bg-white flex items-center justify-center p-4 overflow-hidden font-sans">
      ...
      >
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Pilih Bidang</h2>
          <p className="text-slate-500 mt-2 text-sm">Sesuaikan kategori bisnis untuk memfilter tender Anda.</p>
        </div>
          ...
          {DAFTAR_BIDANG.map((bidang) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-bold text-sm tracking-tight uppercase">{bidang}</span>
              </motion.label>
            );
          })}
        </motion.div>
